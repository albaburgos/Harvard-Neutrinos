...
def effective_area_from_sensitivity(E_pts, E2Phi_pts, *, E_units="GeV", T_years, mu90=2.3, DeltaOmega=2*np.pi):
    """Convert a (E, E^2 Phi)_sens curve at 90% C.L. into effective area A_eff(E).

    Parameters
    ----------
    E_pts : array-like
        Energy points.
    E2Phi_pts : array-like
        Sensitivity curve values E^2 * Phi (GeV cm^-2 s^-1 sr^-1) at the E_pts.
    E_units : {"GeV", "eV"}
        Units of E_pts.
    T_years : float
        Live time in years used by the sensitivity curve.
    mu90 : float
        Feldman-Cousins μ90 (e.g., 2.3 or 2.44).
    DeltaOmega : float
        Solid angle (sr) corresponding to the sensitivity curve.

    Returns
    -------
    E_GeV : np.ndarray
        Energies in GeV (sorted ascending).
    Aeff_m2 : np.ndarray
        Effective area in m^2 such that N = ∫ φ A_eff ΔΩ T dE.
    """
...
# ------------------------------ Flux model ---------------------------------
class FluxModel:
    """Per-flavor flux model φ(E) [GeV^-1 cm^-2 s^-1 sr^-1].

    Default: φ(E) = φ0 * (E/E0)^(-γ), with
        φ0 = 1.44e-18 GeV^-1 cm^-2 s^-1 sr^-1 (per flavor),
        E0 = 1e5 GeV,
        γ  = 2.0

    The same normalization is applied to each flavor unless specified.
    """

    def __init__(self, phi0_per_flavor=1.44e-18/3, gamma=2.0, E0=1e3):
        self.phi0_per_flavor = float(phi0_per_flavor)
        self.gamma = float(gamma)
        self.E0 = float(E0)

    def phi(self, E_GeV, flavor=None):
        """Return flux φ(E) for a given energy or array of energies (GeV)."""
...
# Flavor fractions for radio detection (from external CSV). Fallback to 1/3 each.
fe_g = np.ones_like(E_grid_radio) / 3
fmu_g = np.ones_like(E_grid_radio) / 3
ft_g = np.ones_like(E_grid_radio) / 3
...
# ------------------------------ TAMBO (AΩ) ---------------------------------
# T = 10 yr, aperture AΩ given in m^2·sr for all and tau channels. Electron = all - tau
T_tambo_years = 10.0
omega_tambo   = 2.0       # sr
...
# The user's paste had typos; we attempt robust parsing below. You can replace
# `raw_track_bins` with your own clean list.
raw_track_bins = [
    "10e2 - 9e3", "1.5e2 - 1.2e4", "2e2 - 1.8e4", "2.1e2 - 2.2e4",
    "3e2 - 3e4", "3.5e2- 4e4", "4.5e2 - 5.6e4", "5.5e2 - 6e4",
    "7e2 - 7.5e4", "9e2 - 7e4", "1.2e3 - 6e4", "1.5e3 - 5.2e4",
    "2e3 - 4e4", "2.1e3 - 3e4", "3e3 - 2e4", "3.5e3 - 1.5e4",
    "4.5e3 - 1e4", "6e3 - 6.5e3", "7e3 - 4.5e3", "9e3 - 3e3",
    "1.2e4 - 2e3", "1.5e4 - 1.5e3", "2e4 - 1e3", "2e4 - 7e2",
    "3e4 - 4.5e2", "3.8e4 - 3.5e2", "4.5e4 - 2e2", "6e4 - 1.5e2",
    "7e4 - 7e1", "9e4 - 6e1", "1e5 - 4e1", "1.5e5 - 3e1",
    "1.8e5 - 2e1", "2e5 - 1e1", "3e5 - 3.5e0", "3.5e5 - 7e0",
    "4.5e5 - 4e0", "6e5 - 3e0", "7e5 - 3.4e0", "9e5 - 2e0",
    "1.2e6 - 1e0", "4.5e6 - 1e0",
]

def _parse_track_bin(s):
    s = s.replace(" ", "")
    if "-" not in s:
        return None
    lo, hi = s.split("-")
    # left is energy low, right is count N (based on user's phrasing)
    def _eval_token(tok):
        # accept forms like 1e3, 10e2, 3.5e0, or bare integers
        if "e" in tok:
            parts = tok.split("e")
            if parts[0] == "":
                base = 1.0
                ...
                base = float(parts[0])
            expo = int(float(parts[1]))
            return base * (10.0 ** expo)
        return float(tok)
    try:
        E_low = _eval_token(lo)
        N_bin = _eval_token(hi)
        return (E_low, N_bin)
    except Exception:
        return None

IC_track_bins = [x for x in (_parse_track_bin(s) for s in raw_track_bins) if x]
IC_track_bins.sort(key=lambda t: t[0])
IC_track_T_years = 6.0
TAU_TRACK_SPILL_ABOVE = 1e6  # GeV: split τ into track channel above this energy
TAU_TRACK_FRACTION   = 0.1   # fraction of track counts attributed to τ above threshold
...
X_tau= add_exposure_from_AOm(E_tambo, AOm_tambo_tau, T_years=T_tambo_years, accum=X_tau)
...
# ------------------------------ Event counts --------------------------------
flux = FluxModel(phi0_per_flavor=1.44e-12, gamma=2.0, E0=1e2)  
...
'''
# Add IceCube Tracks direct counts to muon/tau decades
for E_low, Nbin in IC_track_bins:
    # Determine which decade bin this E_low falls into (assume narrow bins)
    # If your bins are wide, consider using explicit (E_low,E_high) to slice.
    k = np.searchsorted(decade_edges, E_low, side='right') - 1
    if k < 0 or k >= len(N_mu_dec):
        continue
    if E_low >= TAU_TRACK_SPILL_ABOVE:
        N_tau_dec[k] += TAU_TRACK_FRACTION * Nbin
        N_mu_dec[k]  += (1.0 - TAU_TRACK_FRACTION) * Nbin
    else:
        N_mu_dec[k] += Nbin
