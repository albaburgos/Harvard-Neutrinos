...
def make_radio_plot(icecube_csv_path="friday.csv",
    ...
    col_radio_tau = '#2ca02c'  # green
    col_poemma    = '#e41a1c'  # red
    col_trinity   = '#984ea3'  # purple
    col_retn_lo   = '#ff7f00'  # orange (for band edges)
    col_retn_hi   = '#ff7f00'
    col_retn_fill = '#ffdf80'  # light orange fill
    col_tambo_tau = '#4d4d4d'  # gray
    col_tambo_all = '#7f7f7f'  # lighter gray
    col_ic_mu     = '#f29b38'  # lighter shade of muon
    col_ic_el     = '#1f77b2'  # match electron family
    col_ic_tau    = '#2ca02c'  # match tau family
    ...
    ax.plot(E_po_GeV, A_po, label="POEMMA (Aeff)", color=col_poemma)
    ax.scatter(E_po_GeV, A_po, color=col_poemma)
    ax.plot(E_tr_GeV, A_tr, label="Trinity (Aeff)", color=col_trinity)
