...
CSV_PATHT = "MC_outputs/effareasTambo.csv"
BIN_WIDTH_LOG10 = 0.2
BIN_WIDTH_Tambo = 4
PHI0 = 2.72e-18
...
def integrate_bin_I(edges: np.ndarray, centers: np.ndarray, A: np.ndarray, Emin: float, Emax: float) -> float:
    ...
    for j in range(len(centers)):
        ...
        U = min(b, Emax)
        if L < U:
            total += A[j] * ((centers[j]/E0)**-2.54) * (dE[j]) * T_EXPOSURE * OMEGA * PHI0 
    return float(total)
...
def main():
    ...
    ET, A_muT, A_tauT, A_eT = read_effarea_csv(CSV_PATHT)
    edges = geometric_edges_from_centers(E)
    edgesT = geometric_edges_from_centers(ET)
    edgesall = np.concatenate((edges, edgesT))
    coarse_edges = make_log_bins(edgesall, BIN_WIDTH_LOG10)
    norm = 2

    # Baseline distribution (1/3,1/3,1/3)
    fe0 = fmu0 = ftau0 = 1.0/3.0
    base_countsIC = events_per_coarse_bin(
        E, edges, A_e, A_mu, A_tau, coarse_edges, fe0, fmu0, ftau0, norm
    )

    coarse_edgesT = make_log_bins(edgesT, BIN_WIDTH_Tambo)

    base_countsT = events_per_coarse_bin(
        ET, edgesT, A_eT, A_muT, A_tauT, coarse_edgesT, fe0, fmu0, ftau0, norm
    )

    base_counts = base_countsIC
    base_counts[12:] = base_countsIC[12:] + base_countsT[0]*(BIN_WIDTH_LOG10/BIN_WIDTH_Tambo)

    counts_001IC = events_per_coarse_bin(E, edges, A_e, A_mu, A_tau, coarse_edges, 0.0, 0.0, 1.0, norm)
    counts_010IC = events_per_coarse_bin(E, edges, A_e, A_mu, A_tau, coarse_edges, 0.0, 1.0, 0.0, norm)
    counts_100IC = events_per_coarse_bin(E, edges, A_e, A_mu, A_tau, coarse_edges, 1.0, 0.0, 0.0, norm)

    counts_001T = events_per_coarse_bin(ET, edgesT, A_eT, A_muT, A_tauT, coarse_edgesT, 0.0, 0.0, 1.0, norm)
    counts_010T = events_per_coarse_bin(ET, edgesT, A_eT, A_muT, A_tauT, coarse_edgesT, 0.0, 1.0, 0.0, norm)
    counts_100T = events_per_coarse_bin(ET, edgesT, A_eT, A_muT, A_tauT, coarse_edgesT, 1.0, 0.0, 0.0, norm)

    counts_001 = counts_001IC + counts_001T[0]*(BIN_WIDTH_LOG10/BIN_WIDTH_Tambo)
    counts_010 = counts_010IC + counts_010T[0]*(BIN_WIDTH_LOG10/BIN_WIDTH_Tambo)
    counts_100 = counts_100IC + counts_100T[0]*(BIN_WIDTH_LOG10/BIN_WIDTH_Tambo)

    counts_001[12:] = counts_001IC[12:] + counts_001T[0]*(BIN_WIDTH_LOG10/BIN_WIDTH_Tambo)
    counts_010[12:] = counts_010IC[12:] + counts_010T[0]*(BIN_WIDTH_LOG10/BIN_WIDTH_Tambo)
    counts_100[12:] = counts_100IC[12:] + counts_100T[0]*(BIN_WIDTH_LOG10/BIN_WIDTH_Tambo)

    counts_listplot = [counts_001IC, counts_010IC, counts_100IC, base_countsIC]
    ...
    counts_listplotT = [counts_001T, counts_010T, counts_100T, base_countsT]
    plot_2x2_histograms(coarse_edgesT,
                         counts_listplotT,
                         labels_plot,
                         "2x2histT.png")
    
    counts_listplotA = [counts_001, counts_010, counts_100, base_counts]
    plot_2x2_histograms(coarse_edges,
                         counts_listplotA,
                         labels_plot,
                         "2x2histAll.png")
    ...
    for i, (fe, fmu, ftau) in enumerate(flavors):
        countsIC = events_per_coarse_bin(
            E, edges, A_e, A_mu, A_tau, coarse_edges, fe, fmu, ftau, norm
        )
        countsT = events_per_coarse_bin(
            ET, edgesT, A_eT, A_muT, A_tauT, coarse_edgesT, fe, fmu, ftau, norm
        )
        counts = countsIC
        print(len(countsIC))
        counts[12:] = countsIC[12:]  + countsT[0]*(1/7)
    ...
    ax.axis("off")
    ax.set_title('\nFlavor Likelihood Analysis with MESE (11.3 yr) + Tambo (10 yr) single energy bin'
