...
def main():
    ...
    coarse_edgesT = make_log_bins(edgesT, BIN_WIDTH_LOG10T)
    norm = 2
    ...
    counts_listplot = [base_counts, base_countsT, counts_100, base_counts]
    labels_plot = ["IC", "Tambo", "100", "base"]
    ...
    for i, (fe, fmu, ftau) in enumerate(flavors):
        ...
        counts_listIC.append(counts)    # add to Python list
    ...
    for i, (fe, fmu, ftau) in enumerate(flavors):
        ...
        countsT = np.asarray(countsT)   # ensure NumPy array
        counts_listT.append(countsT)    # add to Python list

    ll_gridIC = np.array([poisson_loglike(base_counts, lam) for lam in counts_listIC])
    ll_gridT = np.array([poisson_loglike(base_countsT, lamT) for lamT in counts_listT])
    ll_grid = ll_gridIC+ll_gridT
    ...
    ax.axis("off")
    ax.set_title('\nFlavor Likelihood Analysis IC (11.3yr) + TAMBO (10yr) single energy bin'
